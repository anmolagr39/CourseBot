...
# Define the LLaMA 8B model through the Groq API
def get_llama_response(user_prompt):
    try:
        ...
        # Hardcoded Groq API key for LLaMA model
        client = Groq(api_key="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")

        # Groq API call for LLaMA 8B model
        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "user",
                    "content": full_prompt,  # Using full_prompt as the query
                }
            ],
            model="llama3-8b-8192",  # Specifying the LLaMA 8B model
        )
...
def query_chroma_db(query_text, n_result=1):
    results = collection.query(
        query_texts=[query_text],  # Provide the query text from LLaMA 8B model
        n_results=n_result,
        include=["metadatas"]
    )
...
# Groq LLaMA 3 API Call (Using Groq to process the query and Chroma DB result)
def process_with_llama3(original_query, chroma_db_content):
    ...
    try:
        # Groq API call for LLaMA 3 model (70B)
        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "user",
...
# Merged Pipeline (LLaMA 8B + Chroma DB + Groq LLaMA 3)
def query_pipeline(user_query, last_context=None):
    # 1. Condense the query using LLaMA 8B model
    condensed_query = get_llama_response(user_query)

    # If LLaMA 8B returns a new identifier, update the context
